                ...
                <form method="POST" action="{{ $isEdit ? route('barang-masuk.update', $item->id) : route('barang-masuk.store') }}" class="form grid gap-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div class="grid gap-3">
                            ...
                            <div id="select-supplier" class="select">
                                <button type="button" class="btn-outline justify-between font-normal w-full" id="select-supplier-trigger" aria-haspopup="listbox" aria-expanded="false" aria-controls="select-supplier-listbox">
                                    <span class="truncate">{{ old('supplier_id', $isEdit ? $item->supplier_id : '') ? $suppliers->where('id', old('supplier_id', $isEdit ? $item->supplier_id : ''))->first()->nama_supplier : 'Pilih...'  }}</span>
                                ...
                                <div id="select-supplier-popover" data-popover aria-hidden="true" style="max-height: 320px; overflow: hidden;">
                                    ...
                                    <div role="group" aria-labelledby="group-label-select-supplier-items-1">
                                        ...
                                        <div id="select-supplier-items-1-{{ $k->id }}" role="option" data-value="{{ $k->id }}" {{ old('supplier_id') == $k->id ? 'aria-selected="true"' : '' }} data-keywords="{{ $k->nama_kategori }}">
                                            {{ $k->nama_supplier }}
                                        </div>
                                        @endforeach
                                    </div>
                                ...
                                <input type="hidden" name="supplier_id" value="{{ old('supplier_id', $isEdit ? $item->supplier_id : '') }}" />
                            </div>
