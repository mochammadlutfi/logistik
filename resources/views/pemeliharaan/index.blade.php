<x-app-layout>
    <div class="p-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between gap-2 mb-6">
            <h1 class="text-xl font-semibold">Pemeliharaan Barang</h1>
            <a href="{{ route('pemeliharaan-barang.create') }}" class="btn btn-sm btn-primary">
                <i class="fas fa-plus"></i>
                Tambah Pemeliharaan Barang
            </a>
        </div>
                                ...
                                <div id="action-dropdown-{{ $item->id }}" class="dropdown-menu">
                                    ...
                                    <div id="action-dropdown-{{ $item->id }}-popover" data-popover aria-hidden="true" data-align="end" class="min-w-56">
                                        ...
                                        <div role="group" aria-labelledby="account-options">
                                            <a role="menuitem" href="{{ route('pemeliharaan-barang.show', $item->id) }}">
                                                Detail
                                            </a>
                                            <a role="menuitem" href="{{ route('pemeliharaan-barang.edit', $item->id) }}">
                                                Ubah
                                            </a>
                                            <form action="{{ route('pemeliharaan-barang.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pemeliharaan ini?');">
                                                @csrf
                                                @method('DELETE')
